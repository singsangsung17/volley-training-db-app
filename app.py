...
# ---- Tab 3: Sessions (場次安排：恢復新增與預計組次名稱) ----
with tab3:
    ...
    with col_manage:
        ...
        with st.expander("新增訓練場次"):
            ...
            new_theme = st.text_input("訓練主題", key="new_s_theme")
            if st.button("確認新增場次", type="primary", use_container_width=True):
                if new_theme:
                    exec_one(con, "INSERT INTO sessions (session_date, theme) VALUES (?, ?)", (str(new_date), new_theme))
                    st.success("已新增場次")
                    st.rerun()
        ...
        if not sessions_df.empty:
            ...
            sid = st.selectbox("選擇操作場次", options=list(s_map.keys()), format_func=lambda x: s_map[x], key="s3_sid")
            ...
            with c2: mins = st.number_input("預計分鐘", min_value=5, step=5, value=20)
            
            # 依要求修改：標籤維持預計組次，移除範例中的「,3組」
    ...
    with col_flow:
        st.markdown("#### 本場訓練流程")
        if not sessions_df.empty:
            # 解決 DatabaseError：確保別名正確
            flow_df = df(con, """
                SELECT sd.drill_order AS No, d.drill_name AS 訓練內容, 
                       sd.duration_minutes || ' min' AS 分鐘, sd.target_reps AS 預計組次
                FROM session_drills sd
                JOIN drills d ON d.drill_id = sd.drill_id
                WHERE sd.session_id = ? ORDER BY sd.drill_order ASC
            """, (int(sid),))
